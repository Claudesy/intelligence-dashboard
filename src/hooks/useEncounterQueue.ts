    ...
    ? current.map((e) =>
        e.encounterId === updated.encounterId
          ? {
              ...
              ...updated,
              patientLabel:
                updated.patientLabel === "Pasien"
                  ? e.patientLabel
                  : updated.patientLabel,
...
export function extractEncounterQueueData(
  payload: unknown,
): EncounterQueueItem[] | null {
...
export function useEncounterQueue(
  socketState: Pick<
    ...
    "lastEncounterUpdate" | "isConnected"
  >,
): EncounterQueueState {
  ...
  useEffect(() => {
    ...
    async function fetchEncounters(): Promise<void> {
      try {
        ...
        const json: ApiEncountersResponse =
          (await res.json()) as ApiEncountersResponse;
        ...
        const status = Number(
          error instanceof Error ? Number(error.message) : Number.NaN,
        );
        if (!cancelled) {
          setIsLoading(false);
          setError(
  ...
  useEffect(() => {
    ...
    const item: EncounterQueueItem = {
      ...
      patientLabel:
        typeof data?.patientLabel === "string" ? data.patientLabel : "Pasien",
  ...
  useEffect(() => {
    setIsStale(
      !socketState.isConnected && (!isLoading || encounters.length > 0),
    );
