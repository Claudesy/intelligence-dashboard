 ...
 */
export function consultToAnamnesaPayload(
  consult: AssistConsultPayload,
): AnamnesaFillPayload {
  const ttv = consult.ttv || {};
  const antro = consult.anthropometrics || {};
  const kronis = Array.isArray(consult.penyakit_kronis)
    ? consult.penyakit_kronis
    : [];
  ...
  const payload: AnamnesaFillPayload = {
    ...
    riwayat_penyakit:
      kronis.length > 0
        ? { sekarang: kronis.join(", "), dahulu: "", keluarga: "" }
        : undefined,
    vital_signs: {
  ...
  if (
    ...
    const mappedImt:
      ...
      | "BB Lebih"
      | "Obesitas I"
      | "Obesitas II" = hasilImt.includes("Kurus")
      ? "Kurus"
 ...
 */
export function consultToBridgePayload(
  consult: AssistConsultPayload,
): RMETransferPayload {
  return {
