 ...
 */
export function getRecentBaseline(
  history: HistoricalBP[],
): { sbp: number; dbp: number } | null {
  // Filter for clinic readings only
  const clinicReadings = history
    .filter((r) => !r.location || r.location === "clinic")
    .slice(-3); // Last 3
...
export function detectOccultShock(input: OccultShockInput): OccultShockResult {
  ...
  if (map < SHOCK_THRESHOLDS.MAP_HYPOPERFUSION) {
    triggers.push(
      `MAP ${map} <${SHOCK_THRESHOLDS.MAP_HYPOPERFUSION} (organ hypoperfusion risk)`,
    );
    risk_level = "CRITICAL";
  }
  ...
  if (
    delta_sbp !== undefined &&
    delta_sbp >= SHOCK_THRESHOLDS.RELATIVE_HYPOTENSION_DELTA
  ) {
    triggers.push(
      `Relative hypotension (ΔSBP ${delta_sbp} from baseline ${baseline_sbp})`,
    );
  ...
  return {
    ...
    recommendations,
    baseline_bp:
      baseline_sbp !== undefined
        ? { sbp: baseline_sbp, dbp: baseline_dbp! }
        : undefined,
    delta_sbp,
    map,
  };
...
export function assessPerfusion(assessment: PerfusionAssessment): {
  ...
  if (assessment.capillary_refill_sec > 2) {
    findings.push(
      `Prolonged CRT: ${assessment.capillary_refill_sec} sec (>2 sec)`,
    );
    status = "poor";
  }
  ...
  // Urine output
  if (
    assessment.urine_output_ml_kg_hr !== undefined &&
    assessment.urine_output_ml_kg_hr < 0.5
  ) {
    findings.push(
      `Low urine output: ${assessment.urine_output_ml_kg_hr} mL/kg/hr (<0.5)`,
    );
    status = "poor";
  }
...
export function integratedTTVWorkflow(patient: {
  ...
  if (patient.known_htn && patient.has_acute_symptoms && patient.bp_history) {
    ...
    if (
      shockRisk.risk_level === "CRITICAL" ||
      shockRisk.risk_level === "HIGH"
    ) {
      return {
