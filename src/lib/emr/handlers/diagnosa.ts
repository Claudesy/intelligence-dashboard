...
export async function fillDiagnosa(
  ...
  if (payload.jenis) {
    ...
    const ok = await fillSelectWithFallbacks(
      page,
      jenisSelectors,
      possibleValues,
    );
  ...
  if (payload.prognosa) {
    ...
    const prognosaValue =
      PROGNOSA_EPUSKESMAS_MAP[payload.prognosa] || payload.prognosa;
  ...
  if (payload.penyakit_kronis && payload.penyakit_kronis.length > 0) {
    for (const penyakit of payload.penyakit_kronis) {
      const keywords = CHRONIC_DISEASE_KEYWORDS[penyakit] || [
        penyakit.toLowerCase(),
      ];
