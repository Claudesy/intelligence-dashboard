...
function isEmrSourceOrigin(value: string | null): value is EmrSourceOrigin {
  return (
    value !== null &&
    EMR_SOURCE_ORIGIN_VALUES.includes(value as EmrSourceOrigin)
  );
}

export function resolveEmrSourceInput(
  input: EmrSourceInput,
): ResolvedEmrSourceInput {
...
export function buildEmrSourceQuery(input: EmrSourceInput): string {
  ...
  if (resolved.appointmentId)
    query.set("appointmentId", resolved.appointmentId);
  if (resolved.consultId) query.set("consultId", resolved.consultId);
  if (resolved.bridgeEntryId)
    query.set("bridgeEntryId", resolved.bridgeEntryId);
