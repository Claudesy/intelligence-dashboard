...
function parseUsersFromJson(raw: string): CrewAccessCredential[] {
  ...
  for (const item of parsed) {
    ...
    const passwordHash = String((item as { passwordHash?: unknown }).passwordHash ?? '').trim()
    ...
    const status = String((item as { status?: unknown }).status ?? 'ACTIVE').trim()
    users.push({
      ...
      email: normalizeEmail(
        String((item as { email?: unknown }).email ?? `${username}@local.invalid`)
      ),
      institution: normalizeInstitution((item as { institution?: unknown }).institution),
      profession,
      role: String(
        (item as { role?: unknown }).role ?? deriveCrewRoleFromProfession(profession)
      ).trim(),
      status,
    })
  }
...
function credentialToUserRecord(user: CrewAccessCredential): CrewAccessUserRecord {
...
export async function validateCrewAccess(
  ...
  const found = users.find(u => u.username === normalizedUsername || u.email === normalizedEmail)
...
export function getCrewSessionFromCookieHeader(cookieHeader: string): CrewAccessSession | null {
  try {
    ...
    const users = getCrewAccessUsers()
    const found = users.find(u => u.username === normalizeUsername(payload.username))
    if (!found) return null
...
export async function updateCrewAccessUser(
  ...
  return withUserFileLock(async () => {
    ...
    if (updates.displayName) records[index].displayName = updates.displayName.trim()
    if (updates.email) records[index].email = updates.email.trim().toLowerCase()
    if (updates.institution) records[index].institution = updates.institution.trim()
    if (updates.profession) records[index].profession = updates.profession.trim()
