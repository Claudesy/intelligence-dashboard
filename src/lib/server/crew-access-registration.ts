...
import { resolveCrewProfileAvatarUrl } from "@/lib/crew-profile";
...
function loadRegistrationRequests(): CrewAccessRegistrationRecord[] {
  ...
  try {
    ...
    return Array.isArray(parsed)
      ? (parsed as CrewAccessRegistrationRecord[])
      : [];
  } catch {
    throw new Error(
...
function saveRegistrationRequests(
  requests: CrewAccessRegistrationRecord[],
): void {
  const filePath = getRegistrationRequestsFilePath();
  ...
  while (true) {
    try {
      ...
      if (Date.now() - startedAt > 2000) {
        throw new Error(
          "Sistem pendaftaran sedang sibuk. Silakan coba lagi beberapa saat.",
        );
      }
...
function validateRegistrationPayload(
  ...
  const rawServiceAreas = Array.isArray(body.serviceAreas)
    ? body.serviceAreas
    : [];
  ...
  if (!isCrewAccessProfession(profession)) {
    throw new Error(
      `Profesi harus salah satu dari: ${CREW_ACCESS_PROFESSIONS.join(", ")}.`,
    );
  }
  ...
  if (!isCrewAccessGender(gender)) {
    throw new Error(
      `Jenis kelamin harus salah satu dari: ${CREW_ACCESS_GENDERS.join(", ")}.`,
    );
  }
  ...
  return {
    ...
    serviceAreas,
    serviceAreaOther: serviceAreas.includes("Lainnya")
      ? serviceAreaOther
      : undefined,
  };
}
...
export async function createCrewAccessRegistration(raw: unknown): Promise<{
  ...
  return withRegistrationLock(async () => {
    ...
    if (existingPending) {
      throw new Error(
        "Pendaftaran dengan email atau username ini sudah menunggu review.",
      );
    }
...
export function listPendingRegistrations(): Omit<
  CrewAccessRegistrationRecord,
  "passwordHash"
>[] {
