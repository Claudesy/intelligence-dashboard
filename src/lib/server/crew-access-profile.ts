...
function findCrewProfessionByUsername(username: string): CrewAccessProfession | '' {
  return listCrewAccessUsers().find(user => user.username === username)?.profession ?? ''
}
...
export function listAllCrewProfiles(): Map<string, CrewProfileData> {
  ...
  const users = new Map(listCrewAccessUsers().map(user => [user.username, user]))
