import { validateCDSSOutput, type ValidationResult } from "@abyss/guardrails";
import type {
  CDSSResponse,
  ClinicalAlert,
  IskandarSuggestion,
...
function coerceDiagnosisOption(
  value: unknown,
): AIInsightDiagnosisOption | null {
  if (!isRecord(value)) {
    return null;
  }
...
function coerceSuggestion(
  ...
  const engineVersion =
    typeof value.engineVersion === "string" ? value.engineVersion : "unknown";
  const suggestedAt =
  ...
  const supportingEvidence = Array.isArray(value.supportingEvidence)
    ? value.supportingEvidence.filter(
...
function coerceAlert(value: unknown): ClinicalAlert | null {
  ...
  const actionRequired =
    typeof value.actionRequired === "boolean" ? value.actionRequired : false;
...
function coerceCDSSResponse(
  event: IntelligenceEventPayload,
): CDSSResponse | null {
  const source = isRecord(event.data.response)
    ? event.data.response
    : event.data;
  if (!isRecord(source)) {
    return null;
...
function mapSuggestionView(
  suggestion: IskandarSuggestion,
  index: number,
): AIInsightSuggestionView {
...
export function buildAIInsightsSnapshot(
  ...
  if (isUnavailable(event)) {
    return {
      ...
      isDegraded: true,
      degradedMessage:
        "CDSS tidak tersedia saat ini. Lanjutkan review klinis manual.",
    };
  }
  ...
  const safeSuggestions = response.suggestions.filter(
    (suggestion) => suggestion.confidence >= 0.1,
  );
  ...
  return {
    ...
    suggestions: safeSuggestions.map((suggestion, index) =>
      mapSuggestionView(suggestion, index),
    ),
    validation,
    isIdle: false,
    isDegraded: false,
