...
const INTELLIGENCE_EVENT_STATUS_LABELS: Record<
...
export type IntelligenceEventStatus =
  (typeof INTELLIGENCE_EVENT_STATUSES)[number];

export function isIntelligenceEventStatus(
  value: unknown,
): value is IntelligenceEventStatus {
  return (
    typeof value === "string" &&
...
export function getIntelligenceEventStatusLabel(
  status: IntelligenceEventStatus,
): string {
  return INTELLIGENCE_EVENT_STATUS_LABELS[status];
}

export function isIntelligenceEventPayload(
  payload: unknown,
): payload is IntelligenceEventPayload {
