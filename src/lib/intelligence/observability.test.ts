...
test("observability delivery only marks fingerprints as reported after a successful post", () => {
  ...
  assert.equal(
    canDispatchObservabilityFingerprint(initialState, fingerprint),
    true,
  );

  const pendingState = markObservabilityDeliveryPending(
    initialState,
    fingerprint,
  );
  assert.equal(
    canDispatchObservabilityFingerprint(pendingState, fingerprint),
    false,
  );

  const failedState = markObservabilityDeliveryFailed(
    pendingState,
    fingerprint,
  );
  ...
  assert.equal(
    canDispatchObservabilityFingerprint(failedState, fingerprint),
    true,
  );

  const succeededState = markObservabilityDeliverySucceeded(
    pendingState,
    fingerprint,
  );
  ...
  assert.equal(
    canDispatchObservabilityFingerprint(succeededState, fingerprint),
    false,
  );
});
