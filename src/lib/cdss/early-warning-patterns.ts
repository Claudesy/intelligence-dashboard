...
function checkSepsisPattern(
  ...
  // Infection context
  const hasInfectionContext = /infeksi|demam|sepsis|isk|ispa|pneumoni|selulitis|abses|luka infeksi|peritonitis/i.test(
    [
      input.keluhan_utama,
  ...
  if (sirsCount >= 2 && news2.aggregate_score >= 4) {
    return {
      ...
      action:
        "Evaluasi sumber infeksi, cek lab darah lengkap, monitoring tiap 2 jam",
      criteria_met: [
        ...sirsCriteria,
        `NEWS2 aggregate: ${news2.aggregate_score}`,
      ],
...
function checkHemorrhagicShockPattern(
  ...
  const hasBleedContext = /darah|perdarahan|hematemesis|melena|bab hitam|bab darah|haid banyak|flek|nifas|pasca operasi|trauma/i.test(
    [input.keluhan_utama, input.keluhan_tambahan ?? ""].join(" "),
  );
...
function checkPreeclampsiaPattern(
  ...
  const hasNeuroSymptoms = /sakit kepala|kepala pusing|pandangan kabur|mata berkunang|kejang|gelisah|nyeri ulu hati/i.test(
    [input.keluhan_utama, input.keluhan_tambahan ?? ""].join(" "),
  );
 ...
 */
function checkMalariaPattern(
  input: CDSSEngineInput,
  v: VitalSigns,
): EarlyWarningMatch | null {
  const hasMalariaContext = /malaria|gigitan nyamuk|daerah endemis|demam menggigil|demam periodik/i.test(
    [input.keluhan_utama, input.keluhan_tambahan ?? "", ...(input.chronic_diseases ?? [])].join(" "),
  );
  ...
  if (hypothermia || (tachycardia && hypotension) || lowSpO2) {
    return {
      ...
      action:
        "Artesunate IV/IM jika tersedia, pasang IV line, rujuk SEGERA ke RS",
      criteria_met: [
...
export function earlyWarningsToRedFlags(
  matches: EarlyWarningMatch[],
): Array<{
  severity: "emergency" | "urgent" | "warning";
  condition: string;
  action: string;
