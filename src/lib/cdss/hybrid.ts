...
function hasValidationFlag(
  ...
  return (
    suggestion.validation_flags?.some((flag) => flag.type === type) ?? false
  );
}
...
function redFlagMatchesContext(
  ...
  return redFlags.some((flag) => {
    ...
    const meaningfulTokens = normalizedFlag
      .split(" ")
      .filter(
        (token) => token.length >= 4 && !GENERIC_RED_FLAG_TOKENS.has(token),
      );

    if (meaningfulTokens.length === 0) return false;

    const overlapCount = meaningfulTokens.filter((token) =>
      caseText.includes(token),
    ).length;
...
function explainsChiefComplaint(
  ...
  const complaintTokens = complaint
    .split(" ")
...
export function buildProblemRepresentation(input: CDSSEngineInput): string {
  const snippets = [
    input.assessment_conclusion
      ? `sintesis dokter ${input.assessment_conclusion}`
      : null,
    `${input.usia} tahun`,
    ...
    input.keluhan_tambahan
      ? `keluhan tambahan ${input.keluhan_tambahan}`
      : null,
    input.chronic_diseases?.length
      ? `komorbid ${input.chronic_diseases.join(", ")}`
      : null,
...
export function mergeDiseaseCandidates(
  ...
  const keywordMax = Math.max(
    0,
    ...keywordCandidates.map((candidate) => candidate.score),
  );
...
export function buildNextBestQuestions(
  ...
  if (!input.keluhan_tambahan?.trim()) {
    questions.push(
      "Apa gejala penyerta utama, durasi, dan progresivitas keluhan?",
    );
  }
  ...
  if (
    ...
    questions.push(
      "Konfirmasi kembali usia, jenis kelamin biologis, dan status kehamilan pasien.",
    );
  }

  return [
    ...new Set(questions.filter((question) => question.trim().length > 0)),
  ].slice(0, 5);
}
...
export function applyHybridDecisioning(
  ...
  const scoredSuggestions = validation.suggestions.map((suggestion) => {
    ...
    const byCode =
      candidateByCode.get(suggestion.icd10_code.trim().toUpperCase()) ?? [];
    ...
    let decisionStatus: NonNullable<ValidatedSuggestion["decision_status"]> =
      "deferred";
  ...
  const counts = {
    ...
    review: scoredSuggestions.filter(
    ...
    deferred: scoredSuggestions.filter(
      (suggestion) => suggestion.decision_status === "deferred",
    ).length,
  };
  ...
  const requiresMoreData =
    ...
    scoredSuggestions.every(
      (suggestion) => (suggestion.missing_information?.length ?? 0) > 0,
    );
  ...
  const ordered = [
    ...scoredSuggestions
      ...
      .sort(
        (left, right) =>
          (right.deterministic_score ?? 0) - (left.deterministic_score ?? 0),
      ),
    ...scoredSuggestions
  ...
  return {
    suggestions,
    nextBestQuestions: buildNextBestQuestions(
      input,
      suggestions,
      validation.warnings,
    ),
