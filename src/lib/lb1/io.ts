...
import type { ParsedExport, RawRow, DiagnosisMapping } from "./types";
import {
  CANONICAL_COLUMNS,
  REQUIRED_COLUMNS,
  EPUSKESMAS_COL,
...
function scoreHeaderRow(rowValues: string[]): number {
  ...
  for (const aliases of Object.values(CANONICAL_COLUMNS)) {
    if (
      aliases.some((alias) => rowValues.some((cell) => cell.includes(alias)))
    ) {
      score++;
    }
  }
...
function parseWithHeader(raw: unknown[][], sourcePath: string): ParsedExport {
  ...
  if (score < 3)
    throw new Error("Header score terlalu rendah. Fallback ke positional.");
...
function parseEpuskesmasPositional(
  raw: unknown[][],
  sourcePath: string,
): ParsedExport {
  ...
  for (let i = 0; i < raw.length; i++) {
    ...
    for (const dxRaw of [dx1, dx2, dx3]) {
      if (
        !dxRaw ||
        dxRaw.toUpperCase() === "NAN" ||
        dxRaw.toUpperCase() === "NONE"
      )
        continue;
...
function parseRegisterFormat(
  ...
  const headerRow = (raw[headerRowIdx] as unknown[]).map((v) =>
    String(v ?? "").trim(),
  );
  ...
  for (let i = headerRowIdx + 1; i < raw.length; i++) {
    ...
    const umurThnNum = parseInt(umurThnRaw, 10) || 0;
    const umurThn = umurThnNum === 0 ? "1" : umurThnRaw;
    const jenisKunj =
      colJenisKunj >= 0 ? String(r[colJenisKunj] ?? "").trim() : "";
    ...
    for (let d = 0; d < 5; d++) {
      ...
      const visitType =
        jenisKunj || String(jkCols[d] >= 0 ? (r[jkCols[d]] ?? "") : "").trim();
