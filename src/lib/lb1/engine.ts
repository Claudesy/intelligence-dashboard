...
import { loadLb1Config, resolveLb1RuntimePaths, pathExists } from "./config";
import {
  parseExportFile,
  readDiagnosisMapping,
  parseRegisterRujukan,
} from "./io";
import {
  normalizeRecords,
  aggregateForLb1,
...
async function listExportCandidates(
  ...
  const filesWithMeta = (
    ...
    (entry): entry is { name: string; fullPath: string; mtimeMs: number } =>
      Boolean(entry),
  );
...
async function tryParseExportCandidate(filePath: string): Promise<{
  ...
  for (let attempt = 1; attempt <= maxAttempts; attempt++) {
    try {
      ...
      const shouldRetry =
        isFileAccessError(lastErrorMessage) && attempt < maxAttempts;
      if (shouldRetry) {
...
function toLb1SummaryFromUnknown(payload: unknown): LB1Summary | null {
  ...
  if (
    ...
    return {
      ...
      unmappedDx: Array.isArray(data.unmappedDx)
        ? data.unmappedDx.map((x) => String(x))
        : [],
  ...
  if (
    Number.isFinite(snakeYear) &&
    Number.isFinite(snakeMonth) &&
    Number.isFinite(snakeValid)
  ) {
    return {
      ...
      unmappedDx: Array.isArray(data.unmapped_dx)
        ? data.unmapped_dx.map((x) => String(x))
        : [],
...
export async function runLb1Engine(
  ...
  try {
    ...
    const { dataDir, outputDir, templatePath, mappingPath } =
      resolveLb1Paths(config);

    // Pipeline config (dengan defaults)
    const stripIcdSubcode = parseBoolean(
      config?.pipeline?.strip_icd_subcode,
      true,
    );
    const excludeCodes = normalizeExcludeCodes(config?.pipeline?.exclude_codes);
    const [validDayMin, validDayMax] = parseValidDayRange(
      config?.pipeline?.valid_date_range,
    );
    ...
    if (!parsed) {
      const attemptedList =
        attemptedFiles.length > 0 ? attemptedFiles.join("; ") : dataDir;
    ...
    if (
      parsed.sourcePath &&
      (parsed.sourcePath.endsWith(".xlsx") ||
        parsed.sourcePath.endsWith(".xls"))
    ) {
      try {
    ...
    const validKbCount = normalized.valid.filter(
      (r) => r.visit_type === "KB",
    ).length;
    const validKlCount = normalized.valid.filter(
      (r) => r.visit_type === "KL",
    ).length;
...
export async function loadLatestSummary(): Promise<LB1Summary | null> {
  try {
    ...
    const summaryFiles = files.filter((f) =>
      /^SUMMARY_\d{4}_\d{2}\.json$/.test(f),
    );
