...
export type GlobalDeteriorationState =
  | "improving"
  | "stable"
  | "deteriorating"
  | "critical";
...
function calculateDeviationScore(
  ...
  if (value <= 0) return 0;
  if (value < range.min)
    return round(((range.min - value) / range.min) * 100, 2);
  if (value > range.max)
    return round(((value - range.max) / range.max) * 100, 2);
  return 0;
}
...
function detectTrend(
  ...
  const deviationSeries = values.map((value) =>
    calculateDeviationScore(value, range),
  );
...
function generateNote(
  ...
  if (trend === "insufficient_data")
    return "Data tidak cukup untuk analisis trend.";
...
function calculateTrajectoryVolatility(
  ...
  const volatility = clamp(
    round(avgCV + flipPenalty + burdenPenalty, 1),
    0,
    100,
  );
  ...
  if (overallTrend === "stable") {
    stability =
      volatility <= 25 && burden.total_breaches_last5 <= 1
        ? "true_stable"
        : "pseudo_stable";
  }
...
function calculateAcuteAttackRisk24h(
  ...
  const sbpTrend =
    vitalTrends.find((t) => t.parameter === "sbp")?.trend ??
    "insufficient_data";
  const dbpTrend =
    vitalTrends.find((t) => t.parameter === "dbp")?.trend ??
    "insufficient_data";
  const glucoseTrend =
    vitalTrends.find((t) => t.parameter === "glucose")?.trend ??
    "insufficient_data";
  ...
  else if (vt.glucose >= 300 || (vt.glucose > 0 && vt.glucose < 70))
    glycemic = 78;
...
function estimateTimeToCriticalForVital(
  ...
  if (
    ...
    last.value < thresholds.high
  ) {
    eta = (thresholds.high - last.value) / slopePerHour;
  } else if (
    slopePerHour < 0 &&
    thresholds.low !== undefined &&
    last.value > thresholds.low
  ) {
    eta = (thresholds.low - last.value) / slopePerHour;
  }

  if (eta === null || !Number.isFinite(eta) || eta < 0 || eta > MAX_ETA_HOURS)
    return null;
  return round(eta, 1);
}

function calculateTimeToCriticalEstimate(
  visits: VisitRecord[],
...
function deriveDrivers(
  ...
  if (!latestVisit)
    return ["Data kunjungan tidak tersedia untuk menentukan driver risiko."];

  const vt = latestVisit.vitals;

  if (vt.sbp >= 180 || vt.dbp >= 120)
    drivers.push("Tekanan darah berada pada zona krisis.");
  else if (vt.sbp >= 160 || vt.dbp >= 100)
    drivers.push("Tekanan darah tinggi persisten pada rentang risiko tinggi.");

  if (vt.glucose >= 300)
    drivers.push(
      "Gula darah sewaktu sangat tinggi, risiko dekompensasi metabolik meningkat.",
    );
  ...
  if (vt.temp >= 38.5 && vt.hr > 100 && vt.rr >= 22) {
    drivers.push(
      "Pola demam + takikardi + takipnea mengarah ke perburukan sistemik.",
    );
  }
  ...
  const unresolvedDeclines = vitalTrends.filter(
    (trend) =>
      ...
      (trend.risk === "high" || trend.risk === "critical"),
  );
  ...
  if (volatility.stability_label === "pseudo_stable") {
    drivers.push(
      "Trajektori tampak stabil namun volatilitas tinggi (pseudo-stable).",
    );
  }

  if (acuteRisk.stroke_acs_suspicion_risk >= 70) {
    drivers.push(
      "Kombinasi gejala + hemodinamik memberi kecurigaan stroke/ACS berbasis rule.",
    );
  }

  if (drivers.length === 0) {
    drivers.push(
      "Tidak ada driver risiko dominan; lanjutkan monitoring berkala.",
    );
  }
...
function determineUrgency(tier: MortalityProxyTier): {
  ...
  if (tier === "moderate") {
    return {
      urgency: "moderate",
      action:
        "REVIEW SAME DAY: Review klinis pada hari yang sama dan evaluasi respons intervensi.",
      risk: "moderate",
    };
  }
  return {
    urgency: "low",
    action:
      "ROUTINE 24H: Monitoring rutin 24 jam dengan edukasi pasien dan follow-up terjadwal.",
    risk: "low",
  };
}
...
function buildRecommendations(
  ...
  recs.push({
    category: "action",
    priority:
      safeOutput.risk_tier === "critical" || safeOutput.risk_tier === "high"
        ? "high"
        : "medium",
...
function deriveOverallTrend(
  ...
  const decliningCount = activeTrends.filter(
    (t) => t.trend === "declining",
  ).length;
  const improvingCount = activeTrends.filter(
    (t) => t.trend === "improving",
  ).length;
...
function buildSummary(
  ...
  const improvingVitals = vitalTrends.filter(
    (v) => v.trend === "improving" && v.values.length > 0,
  );
  const decliningVitals = vitalTrends.filter(
    (v) => v.trend === "declining" && v.values.length > 0,
  );
  const criticalVitals = vitalTrends.filter(
  ...
  if (global.state === "critical") {
    stateText =
      "Kondisi klinis berada pada zona kritis dengan multiple parameter abnormal.";
    ...
    if (score < 30 && breaches === 0) {
      stateText =
        "Perbaikan klinis signifikan dengan normalisasi parameter vital, pasien stabil.";
      ...
      stateText =
        "Perbaikan parsial namun masih ada parameter kritis yang memerlukan atensi.";
    } else {
      stateText =
    ...
    if (breaches === 0 && score < 30) {
      stateText =
        "Kondisi klinis stabil optimal, semua parameter dalam batas normal.";
    } else if (criticalVitals.length > 0) {
      const param = criticalVitals[0].label;
      stateText = `Kondisi relatif stabil namun ${param} tetap memerlukan monitoring ketat.`;
    } else {
      stateText =
        "Kondisi klinis stabil dengan kontrol parameter vital terkendali.";
    }
  }
...
function extractConfirmedChronicDiagnoses(
  visits: VisitRecord[],
): ConfirmedChronicDiagnosis[] {
  const candidates: Array<
    ConfirmedChronicDiagnosis & { hasReadableName: boolean }
  > = [];
  ...
  for (const visit of visits) {
    ...
    const normalizedName = hasReadableName
      ? diagnosis.nama.trim()
    ...
    occurrenceByTypeAndCode.set(
      key,
      (occurrenceByTypeAndCode.get(key) || 0) + 1,
    );
  }
...
export function analyzeTrajectory(visits: VisitRecord[]): TrajectoryAnalysis {
  ...
  const volatility = calculateTrajectoryVolatility(
    vitalTrends,
    overallTrend,
    burden,
  );
  ...
  const decliningCount = vitalTrends.filter(
    (t) => t.trend === "declining",
  ).length;
  const improvingCount = vitalTrends.filter(
    (t) => t.trend === "improving",
  ).length;
  ...
  else if (overallTrend === "improving" && deteriorationScore <= 35)
    globalState = "improving";
  ...
  if (
    acuteRisk.shock_decompensation_risk >= 70 ||
    acuteRisk.sepsis_like_deterioration_risk >= 70
  ) {
    mortalityScore += 10;
  }
