...
test("trajectory reads hypotension that moves toward normal as improving", () => {
  ...
  const systolicTrend = analysis.vitalTrends.find(
    (trend) => trend.parameter === "sbp",
  );
  const diastolicTrend = analysis.vitalTrends.find(
    (trend) => trend.parameter === "dbp",
  );
...
test("trajectory weights severe worsening vitals above mild improvements", () => {
  ...
  const glucoseTrend = analysis.vitalTrends.find(
    (trend) => trend.parameter === "glucose",
  );

  assert.equal(glucoseTrend?.trend, "declining");
  assert.equal(analysis.overallTrend, "declining");
  assert.ok(
    analysis.clinical_safe_output.drivers.some((driver) =>
      driver.includes("Gula Darah Sewaktu"),
    ),
  );
