...
function upsertSuggestion(
  target: Map<string, SuggestionAggregate>,
  raw: RawDiseaseDrug,
) {
  ...
  const key = normalize(
    [canonicalName ?? raw.drug, dose, frequency, route].join(" "),
  );
  const aliases = [
    ...new Set([...(resolved.aliases ?? []), raw.drug].filter(Boolean)),
  ];
  const searchTerms = [
    ...new Set(
  ...
  if (existing) {
    ...
    existing.searchTerms = [
      ...new Set([...existing.searchTerms, ...searchTerms]),
    ];
    return;
  }
...
const suggestionCatalog = (() => {
  ...
  const diseases =
    (diseaseDatabase as { diseases?: RawDiseaseEntry[] }).diseases ?? [];

  diseases.forEach((disease) => {
    disease.pharmacotherapy?.first_line?.forEach((item) =>
      upsertSuggestion(catalog, item),
    );
    disease.pharmacotherapy?.second_line?.forEach((item) =>
      upsertSuggestion(catalog, item),
    );
    disease.pharmacotherapy?.prophylaxis?.forEach((item) =>
      upsertSuggestion(catalog, item),
    );
  });
  ...
  const scored = suggestionCatalog
    .map((item) => {
      ...
      if (
        ...
        item.searchTerms.some((term) => term.startsWith(normalizedQuery))
      ) {
        score += 120;
      } else if (
    ...
    .filter(
      (entry): entry is { item: SuggestionAggregate; score: number } =>
        entry !== null,
    )
    .sort(
      (a, b) =>
