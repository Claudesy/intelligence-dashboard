...
function parseClamlCategories(xmlContent: string): ParsedIcdEntry[] {
  ...
  const classRegex =
    /<Class code="([^"]+)" kind="category">([\s\S]*?)<\/Class>/g;
  ...
  while ((match = classRegex.exec(xmlContent)) !== null) {
    ...
    const labelMatch = preferredRubricMatch[1].match(
      /<Label[^>]*>([\s\S]*?)<\/Label>/i,
    );
...
function buildVersionCatalogFromJson(
  version: IcdVersionKey,
  jsonPath: string,
): VersionCatalog {
...
export function lookupIcdDynamically(query: string): IcdLookupResponse {
  ...
  const rows: IcdConversionItem[] = modernCodes.map((modernRaw) => {
    ...
    const legacyEntry =
      legacyExactEntry ?? findBestMatch(v2010, preferredLegacyCode);
    const legacy =
  ...
  if (transformed.primaryCode)
    tokens.add(transformed.primaryCode.toLowerCase());
  for (const variant of buildCodeVariants(transformed.primaryCode)) {
    tokens.add(variant.toLowerCase());
  }
  for (const candidate of transformed.candidateCodes)
    tokens.add(candidate.toLowerCase());
    ...
    tokenList.length === 0
      ...
      : searchCatalog.entries.filter((entry) => {
          ...
          return tokenList.some(
            (token) => code.includes(token) || title.includes(token),
          );
        });
