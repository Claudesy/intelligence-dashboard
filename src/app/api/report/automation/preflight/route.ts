...
export async function GET(request: Request) {
  ...
  try {
    ...
    const mode = parseMode(
      (url.searchParams.get("mode") || "full-cycle").trim().toLowerCase(),
    );
    ...
    if (mode === "pipeline") {
      if (!dataDirExists)
        issues.push(`Data source dir tidak ditemukan: ${dataDir}`);
      ...
      if (!rmeBaseUrl)
        issues.push("EMR base URL belum dikonfigurasi untuk mode full-cycle.");
      if (!rmeCredentialsAvailable) {
        issues.push(
          `Credential EMR belum tersedia di env (${usernameEnv}, ${passwordEnv}).`,
        );
      }
      if (limitedCandidates.length > 0 && !parserOk) {
        warnings.push(
