...
export async function GET(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (!isCrewAuthorizedRequest(request)) {
    console.warn(`[Bridge] GET /api/emr/bridge/[id] — 401 — correlationId: ${correlationId}`)
  ...
  if (!entry) {
    return NextResponse.json({ ok: false, error: 'Entry tidak ditemukan.' }, { status: 404 })
  }
...
export async function PATCH(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (!isCrewAuthorizedRequest(request)) {
    console.warn(`[Bridge] PATCH /api/emr/bridge/[id] — 401 — correlationId: ${correlationId}`)
    return NextResponse.json({ ok: false, error: 'Unauthorized' }, { status: 401 })
  }
  ...
  try {
    ...
    return NextResponse.json({ ok: false, error: 'Gagal memperbarui entry.' }, { status: 500 })
  }
}
