...
export async function PUT(
  ...
  try {
    ...
    if (!body.name?.trim()) {
      return NextResponse.json(
        { ok: false, error: "Nama institusi wajib diisi." },
        { status: 400 },
      );
    }
    ...
    const msg =
      error instanceof Error ? error.message : "Gagal mengubah institusi.";
...
export async function DELETE(
  ...
  try {
    ...
    if (!target) {
      return NextResponse.json(
        { ok: false, error: "Institusi tidak ditemukan." },
        { status: 404 },
      );
    }
    ...
    const msg =
      error instanceof Error ? error.message : "Gagal menghapus institusi.";
    return NextResponse.json({ ok: false, error: msg }, { status: 400 });
  }
}
