...
function computeSummary(
  encounters: DashboardEncounterSummary[],
): EklaimSummaryStats {
...
function EncounterRow({ encounter, expanded, onToggle }: EncounterRowProps) {
  const { eklaimReadiness, patientLabel, status, activeComplianceFailures } =
    encounter;
  const isReady = eklaimReadiness.isReady;
  const allIssues = dedupeIssues(
    eklaimReadiness.blockers,
    ...
    <div className={styles.encounterRow}>
      ...
      >
        ...
        <span className={styles.checkedAt}>
          {formatCheckedAt(eklaimReadiness.checkedAt)}
        </span>
      ...
      {expanded && !isReady && (
        <div className={styles.issueList}>
          {allIssues.length === 0 ? (
            <span className={styles.emptyIssues}>
              Tidak ada detail masalah tersedia.
            </span>
          ) : (
            allIssues.map((issue) => (
              <div
    ...
    async function fetchEncounters() {
      try {
        const res = await fetch(
          "/api/dashboard/intelligence/encounters?limit=100",
          {
            cache: "no-store",
          },
        );
    ...
    <div className={styles.root}>
      ...
      <div>
        ...
        <p className={styles.description}>
          Status kelengkapan dokumentasi encounter untuk pengajuan e-klaim BPJS.
          Masalah ditampilkan per encounter — klik baris untuk melihat detail.
        </p>
      </div>
      ...
      <div className={styles.kpiGrid}>
        ...
        <KpiCard
          value={summary.ready}
          label="Siap e-Klaim"
          green={summary.ready > 0}
        />
        <KpiCard
          value={summary.notReady}
          label="Perlu Perbaikan"
          red={summary.notReady > 0}
        />
      </div>
