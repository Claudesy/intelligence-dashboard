...
export default function AdminOverviewTab({
  ...
  const lb1SuccessEntries = lb1Recent
    ...
    .reverse();
  const lb1Labels = lb1SuccessEntries.map(
  ...
  return (
    <>
            ...
            {[
              ...
              .map((item, i) => (
                ...
                >
                  ...
                  />
                  <span
                    style={{ fontSize: 12, color: "var(--text-main)", flex: 1 }}
                  >
                    {item.detail}
                  </span>
                  <span
      ...
      >
        <StatusDot
          label="SERVER"
          value={`Uptime ${formatUptime(kpi.serverUptimeSeconds)}`}
          ok
        />
        <StatusDot
          label="CREW"
          value={`${kpi.totalCrew} terdaftar`}
          ok={kpi.totalCrew > 0}
        />
        <StatusDot
          label="LB1"
          value={
            kpi.lb1Runs > 0
              ? `${kpi.lb1SuccessRuns}/${kpi.lb1Runs} sukses`
              : "Belum ada run"
          }
