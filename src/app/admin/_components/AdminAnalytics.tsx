...
export default function AdminAnalytics() {
  ...
  const lineLabels = successEntries.map(
    (e) => `${MONTH_NAMES[e.month]} ${e.year}`,
  );
  ...
  const emrSuccessRate =
    kpi.emrTransfers > 0
      ? ((kpi.emrSuccess / kpi.emrTransfers) * 100).toFixed(1)
      : "0";
  const emrSuccessRateClassName =
    Number(emrSuccessRate) >= 90
      ? `${styles.kpiValue} ${styles.kpiValueHealthy}`
      : styles.kpiValue;
      ...
      <div className={styles.kpiRow}>
        <div className={`${styles.card} ${styles.kpiCard}`}>
          <div className={styles.kpiValue}>
            {kpi.lb1TotalVisits.toLocaleString()}
          </div>
        ...
        <div className={`${styles.card} ${styles.kpiCard}`}>
          <div className={styles.kpiValue}>
            {kpi.emrTransfers.toLocaleString()}
          </div>
        ...
        <div className={`${styles.card} ${styles.kpiCard}`}>
          <div className={styles.kpiValue}>
            {formatLatency(kpi.emrAvgLatencyMs)}
          </div>
