...
export default function PatientJoinPage(): React.JSX.Element {
  ...
  useEffect(() => {
    ...
    fetch(`/api/telemedicine/join/${token}`)
      .then((r) => r.json())
      .then(
        (d: { success: boolean; data?: AppointmentInfo; message?: string }) => {
          if (!d.success || !d.data) {
  ...
  const handleJoin = useCallback(async () => {
    ...
    try {
      ...
      if (!data.success || !data.data)
        throw new Error(data.message ?? "Gagal mendapat token");
  ...
  if (state === "loading") {
    return (
      <div style={fullCenter}>
        <Loader2
          size={36}
          style={{ color: "#D47A57", animation: "spin 1s linear infinite" }}
        />
        <p
          style={{
