...
test("KB has diseases with gejala (BUG-1 fix verification)", () => {
  ...
  assert.ok(
    ratio > 0.7,
    `Only ${(ratio * 100).toFixed(0)}% diseases have gejala. Expected > 70%.`,
  );
});
...
test("preFilter: 'demam batuk pilek' returns scored results with gejala signal", () => {
  ...
  assert.ok(
    results[0].score > 0,
    `Top result score should be > 0, got ${results[0].score}`,
  );
});
...
test("preFilter: 'diare' scores higher for GI conditions", () => {
  ...
  const diareRank = results.findIndex(
    (r: FilteredDiseaseResult) =>
      r.nama.toLowerCase().includes("diare") ||
      r.nama.toLowerCase().includes("gastroenter"),
  );
  assert.ok(
...
test("alias: 'panas' expands to include 'demam'", () => {
  ...
  assert.ok(
    expanded.includes("demam"),
    `Expected 'demam' in expansion, got: ${expanded}`,
  );
});
...
test("alias: 'mencret' expands to include 'diare'", () => {
  ...
  assert.ok(
    expanded.includes("diare"),
    `Expected 'diare' in expansion, got: ${expanded}`,
  );
});
...
test("vital sign logic: diastolic >= 120 should be flagged", () => {
  ...
  assert.ok(
    diastolic >= 120,
    "Diastolic >= 120 should trigger hypertensive emergency flag",
  );
});
...
test("vital sign logic: normal diastolic (80) should NOT be flagged", () => {
  ...
  assert.ok(
    !(diastolic >= 120),
    "Normal diastolic should not trigger emergency flag",
  );
});
...
test("scenario: ISPA - demam batuk pilek should surface respiratory infection", () => {
  const results: FilteredDiseaseResult[] = preFilterDiseases(
    "demam batuk pilek hidung tersumbat",
  );
  ...
  assert.ok(
    hasISPA,
    "ISPA/respiratory conditions should appear for common cold symptoms",
  );
});
...
test("scenario: poliuria polidipsia polifagia should surface DM", () => {
  ...
  const hasDM = results.some(
    (r: FilteredDiseaseResult) =>
      r.nama.toLowerCase().includes("diabetes") ||
      r.icd10.startsWith("E1"),
  );
  assert.ok(
    hasDM,
...
test("scenario: gatal kulit, ruam merah should surface dermatitis", () => {
  const results: FilteredDiseaseResult[] = preFilterDiseases(
    "gatal kulit ruam merah bersisik",
  );
...
async function main() {
  ...
  await writeTestReport(
    "test-cdss-engine.txt",
    "═══ CDSS Engine Unit Tests ═══",
    testResults,
  );
