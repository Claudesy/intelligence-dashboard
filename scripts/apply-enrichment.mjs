...
try {
  ...
  console.error(
    "Run enrich-penyakit-clinical.mjs first to generate enrichment data.",
  );
  process.exit(1);
}
...
console.log(
  `Enrichment file generated at: ${enrichmentData._meta.generated_at}`,
);
...
const updated = entries.map((entry) => {
  ...
  if (
    result.enriched_fields.includes("red_flags") &&
    result.data.red_flags?.length
  ) {
    if (!APPLY) {
      console.log(
        `[${entry.id}] ${entry.nama} — red_flags: ${result.data.red_flags.join(", ")}`,
      );
    }
...
copyFileSync(
  SOURCE,
  join(BACKUP_DIR, `penyakit.pre-enrichment.${timestamp}.bak.json`),
);
...
const finalMissingPF = updated.filter(
  (e) => !e.pemeriksaan_fisik?.length,
).length;
