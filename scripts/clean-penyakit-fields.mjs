...
const updatedMetadata = {
  ...raw._metadata,
  description: raw._metadata.description.replace(
    /\d+ Penyakit/,
    `${actualCount} Penyakit`,
  ),
  total_diseases: actualCount,
  with_clinical_data: actualCount,
  last_updated: new Date().toISOString(),
  merged_fields:
    raw._metadata.merged_fields?.filter(
      (f) => f !== "gejala" && f !== "terpi",
    ) ?? [],
};
...
console.log(
  `       Metadata count updated: ${raw._metadata.total_diseases} → ${actualCount}`,
);
