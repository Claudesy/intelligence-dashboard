...
const KELAS_PATTERNS = [
  ...
  {
    pattern:
      /isosorbid dinitrat|isosorbide dinitrate|\bisdn\b|simvastatin|atorvastatin|statin\b/i,
  ...
  {
    pattern:
      /karbamazepin|carbamazepine|fenitoin|phenytoin|asam valproat|valproic/i,
    kelas: "neurologi",
  },
  // Antiparasit
  {
    pattern:
      /albendazol|albendazole|permetrin|permethrin|ivermectin|piperazin|pirantel/i,
    kelas: "antiparasit",
  },
  // Antijamur
  {
  ...
  {
    pattern:
      /epinefrin|epinephrine|adrenalin|adrenaline|atropin sulfat|atropine sulfate/i,
    kelas: "darurat",
  },
  ...
  {
    pattern:
      /pil kb|kontrasepsi|depo progestin|injeksi kontrasepsi|injeksi.*3 bulan/i,
    kelas: "kontrasepsi",
  },
  // Hormon / Obstetri
  {
  ...
  {
    pattern:
      /semen seng fosfat|zinc phosphate cement|\bchkm\b|monoklorkamfer|fenol gliserol/i,
    kelas: "stomatologi",
  },
  // Oftalmologi
  {
...
function extractRute(item) {
  ...
  // Infus
  if (/infus|intravenous drip/.test(name) || /infus/.test(form))
    return "infus_iv";
  ...
  // Rectal
  if (
    /supositoria|suppositoria|rectal tube|rektal/.test(form) ||
    /supositoria|rectal/.test(name)
  )
    return "rektal";
  ...
  // Inhalation
  if (/inhaler|inhalan|cairan ih\b|nebulizer|mdi\b/.test(name))
    return "inhalasi";

  // Ophthalmic (eye)
  if ((/tetes/.test(form) || /tetes|drop/.test(name)) && /mata|eye/.test(name))
    return "oftalmik";
  if ((/salep/.test(form) || /salep/.test(name)) && /mata|eye/.test(name))
    return "oftalmik";

  // Otic (ear)
  if (
    (/tetes/.test(form) || /tetes|drop/.test(name)) &&
    /telinga|ear/.test(name)
  )
    return "otik";
  ...
  // Oral — standard forms
  if (
    /tablet|kapsul|kaplet|sirup|suspensi|serbuk|larutan oral|cairan oral|kunyah/.test(
      form,
    )
  )
    return "oral";
  ...
  if (/tablet|kapsul|kaplet|sirup|suspensi|serbuk|oral/.test(name))
    return "oral";
  ...
  if (/susu|nutricia|nutridrink|\bsgm\b|dangro|infantrini/.test(name))
    return "oral";
...
console.log(
  `Duplicates removed  : ${removed} (codes: ${[...REMOVE_CODES].join(", ")})`,
);
...
copyFileSync(
  SOURCE,
  join(BACKUP_DIR, `obat_data.pre-fornas2023.${ts}.bak.json`),
);
