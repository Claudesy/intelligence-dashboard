...
async function main(): Promise<void> {
  ...
  process.env.DATABASE_URL =
    "postgresql://test:test@localhost:5432/puskesmas_test";
  ...
  const restoreModuleMocks = installModuleMocks({
      ...
      prisma: {
        cDSSAuditLog: {
          create: async ({ data }: { data: Record<string, unknown> }) => {
            cdssAuditEntries.push({
              ...
              outputSummary:
                (data.outputSummary as Record<string, unknown>) ?? {},
              metadata:
          ...
          findMany: async ({
            ...
            return cdssAuditEntries.filter(
              (entry) => !since || entry.timestamp >= since,
            );
          },
        },
        cDSSOutcomeFeedback: {
          create: async ({ data }: { data: Record<string, unknown> }) => {
            cdssOutcomeEntries.push({
              ...
              overrideReason:
                typeof data.overrideReason === "string"
                  ? data.overrideReason
                  : null,
              outcomeConfirmed:
                typeof data.outcomeConfirmed === "boolean"
                  ? data.outcomeConfirmed
                  : null,
            });
            return data;
          },
        ...
        telemedicineAppointment: {
          findUnique: async ({ where }: { where: { id: string } }) => {
            return where.id === dbState.appointment.id
              ? dbState.appointment
              : null;
          },
        ...
        telemedicineSession: {
          upsert: async ({
            ...
            dbState.session = {
              ...
              actualStartAt:
                create.actualStartAt instanceof Date
                  ? create.actualStartAt
                  : null,
            };
        ...
        telemedicineParticipant: {
          upsert: async ({
            ...
            dbState.participant = {
              ...
              joinedAt:
                create.joinedAt instanceof Date ? create.joinedAt : null,
    ...
    "@/lib/cdss/workflow": {
      ...
      getCDSSQualityMetrics: async () => {
        const diagnoseRows = cdssAuditEntries.filter(
          (entry) => entry.action === "DIAGNOSE_RESULT",
        );
        ...
        const mustNotMissSurfaced = diagnoseRows.reduce(
          (sum, entry) =>
            sum + Number(entry.outputSummary.mustNotMissCount ?? 0),
          0,
        );
        return {
          ...
          total_displayed: diagnoseRows.reduce(
            (sum, entry) =>
              sum + Number(entry.outputSummary.totalDisplayed ?? 0),
            0,
          ),
          total_selected: selectionRows.length,
          selection_rate:
            totalRequests > 0 ? selectionRows.length / totalRequests : 0,
          red_flag_trigger_rate:
            totalRequests > 0
              ? diagnoseRows.filter(
                  (entry) => Number(entry.outputSummary.redFlagCount ?? 0) > 0,
          ...
          unverified_icd_avg_count:
            totalRequests > 0
              ? diagnoseRows.reduce(
                  (sum, entry) =>
                    sum + Number(entry.outputSummary.unverifiedCount ?? 0),
                  0,
                ) / totalRequests
              : 0,
          latency_p95_ms:
            diagnoseRows.length > 0
              ? Math.max(...diagnoseRows.map((entry) => entry.latencyMs))
              : 0,
          ...
          override_rate:
            cdssOutcomeEntries.length > 0
              ? cdssOutcomeEntries.filter(
                  (entry) =>
                    Boolean(entry.overrideReason) ||
                    entry.selectedIcd !== entry.finalIcd,
                ).length / cdssOutcomeEntries.length
              : 0,
          concordance_rate:
            cdssOutcomeEntries.length > 0
              ? cdssOutcomeEntries.filter(
                  (entry) => entry.selectedIcd === entry.finalIcd,
                ).length / cdssOutcomeEntries.length
              : 0,
  ...
  try {
    ...
    const doctorPasswordHash =
      await authModule.hashCrewAccessPassword("DoctorPass#202600");
    const nursePasswordHash =
      await authModule.hashCrewAccessPassword("NursePass#202600");
    ...
    const { parseDiagnoseRequestBody } =
      await import("../src/lib/cdss/diagnose-parser");
    const { generateNarrative } =
      await import("../src/lib/narrative-generator");
    const { suggestContextualVitals } =
      await import("../src/lib/ttv-inference");
    const { validateLLMSuggestions } =
      await import("../src/lib/cdss/validation");
    const { applyHybridDecisioning, mergeDiseaseCandidates } =
      await import("../src/lib/cdss/hybrid");
    const autocompleteRoute =
      await import("../src/app/api/cdss/autocomplete/route");
    const diagnoseRoute = await import("../src/app/api/cdss/diagnose/route");
    const suggestionSelectedRoute =
      await import("../src/app/api/cdss/suggestion-selected/route");
    const outcomeFeedbackRoute =
      await import("../src/app/api/cdss/outcome-feedback/route");
    const redFlagAckRoute =
      await import("../src/app/api/cdss/red-flag-ack/route");
    const qualityDashboardRoute =
      await import("../src/app/api/cdss/quality-dashboard/route");
    const telemedicineTokenRoute =
      await import("../src/app/api/telemedicine/token/route");
    ...
    test("TTV assist: keluhan sesak mempertahankan SpO2 sebagai manual-required", () => {
      ...
      assert.deepEqual(advice.manualMeasurementRequired, [
        "sbp",
        "dbp",
        "map",
        "spo2",
    ...
    test("TTV assist: tipe nyeri yang berbeda menghasilkan estimasi nadi yang berbeda", () => {
      ...
      const kolik = suggestContextualVitals(
        "nyeri kolik pinggang menjalar ke lipat paha",
        {
          ageYears: 34,
          sex: "L",
          recentActivity: "resting",
      ...
      assert.ok(
        (muskuloskeletal.values.pulse ?? 0) < (kolik.values.pulse ?? 0),
      );
      assert.ok(
        (muskuloskeletal.values.pulse ?? 0) <= (nyeriDada.values.pulse ?? 0),
      );
      assert.match(
        muskuloskeletal.metadata.pulse?.reasoning ?? "",
        /muskuloskeletal/i,
      );
    ...
    test("Validasi CDSS: ICD yang ada di KB dan nama canonical lolos dengan rag_verified=true", () => {
      ...
      assert.equal(
        validation.suggestions[0]?.diagnosis_name,
    ...
    test("Validasi CDSS: mismatch nama dan konteks demografis ditandai unverified", () => {
      ...
      assert.ok(
        validation.warnings.some((warning) =>
          warning.includes("tidak cocok dengan nama KB"),
        ),
      );
      assert.ok(
        validation.warnings.some((warning) =>
          warning.includes("memerlukan konteks kehamilan"),
        ),
      );
    });
    ...
    test("Validasi CDSS: kode tidak dikenal dan diagnosis anak pada dewasa tercatat di validation_summary", () => {
      ...
      assert.ok(
        unknownCode.warnings.some((warning) =>
          warning.includes("ICD X99 tidak ditemukan"),
        ),
      );
    ...
    test("Hybrid CDSS: duplicate ICD tetap terpisah, must-not-miss dipisah lane, dan next-best-questions tersedia", () => {
      ...
      assert.ok(
        hybrid.suggestions.some(
          (suggestion) =>
            suggestion.decision_status === "recommended" ||
    ...
    test("Autocomplete klinis: nyeri pinggang dipetakan ke chain nyeri pinggang, bukan tenggorokan atau punggung umum", async () => {
      ...
      assert.equal(
        payload.chain.pemeriksaan.fisik.includes("Inspeksi Faring"),
        false,
      );
      assert.equal(
        payload.chain.pemeriksaan.fisik.includes("Palpasi Vertebra"),
        false,
      );
    });
    ...
    await writeTestReport(
      "test-cdss-report.txt",
      "Safety Net Test Report",
      results,
    );
