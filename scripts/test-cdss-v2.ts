...
function readRoute() {
  return readFileSync(join(CWD, 'src/app/api/cdss/autocomplete/route.ts'), 'utf-8')
}
function readPage() {
...
async function main() {
  ...
  await test('AC-01b: preFilterDiseases — sorted by score desc', async () => {
    ...
    for (let i = 1; i < result.length; i++) {
      assert.ok(result[i].score <= result[i - 1].score, 'Harus sorted by score desc')
    }
  })
  ...
  await test('AC-04b: vectors file struktur valid (dimensions=768)', () => {
    ...
    assert.strictEqual(firstVec!.vector!.length, 768, 'Vektor harus 768 dimensi')
  })
  ...
  await test('AC-04d: isEmbeddingReady() — vectors tersedia dan file valid', () => {
    ...
    assert.ok(readyVectors > 100, `Harus ada >100 valid vectors, ada: ${readyVectors}`)
    // Verifikasi isEmbeddingReady logic di source
    const embeddingFilterSrc = readFileSync(join(CWD, 'src/lib/cdss/embedding-filter.ts'), 'utf-8')
    assert.ok(embeddingFilterSrc.includes('isEmbeddingReady'), 'Fungsi isEmbeddingReady harus ada')
    assert.ok(embeddingFilterSrc.includes('vectors.length > 0'), 'Logic: vectors.length > 0')
  })
  ...
  await test('AC-05: embeddingFilterDiseases — source code verifikasi logic', () => {
    ...
    const src = readFileSync(join(CWD, 'src/lib/cdss/embedding-filter.ts'), 'utf-8')
    assert.ok(src.includes('embeddingFilterDiseases'), 'Fungsi utama harus ada')
    assert.ok(src.includes('cosineSimilarity'), 'Harus ada cosineSimilarity')
    assert.ok(src.includes('gemini-embedding-001'), 'Harus pakai gemini-embedding-001')
    assert.ok(src.includes('SEMANTIC_SIMILARITY'), 'taskType harus SEMANTIC_SIMILARITY')
    assert.ok(src.includes('outputDimensionality: 768'), 'Dimensi harus 768')
    assert.ok(src.includes('.sort((a, b) => b.score - a.score)'), 'Harus sorted by score desc')
  })
  ...
  await test('AC-06: autocomplete route — pakai deepseek-chat + struktur response benar', () => {
    ...
    assert.ok(content.includes('suggestions'), 'Response harus include suggestions')
    assert.ok(content.includes('pemeriksaan'), 'Response harus include pemeriksaan')
  ...
  await test('AC-06b: autocomplete route — timeout 8s + AbortSignal', () => {
    ...
    assert.ok(content.includes('AbortSignal.timeout'), 'Harus pakai AbortSignal.timeout')
  })

  await test('AC-06c: autocomplete route — runtime nodejs', () => {
    const content = readRoute()
    assert.ok(content.includes('runtime = "nodejs"'), 'Harus export runtime nodejs')
  })
  ...
  await test('AC-07: autocomplete — guard query < 2 char, return empty', () => {
    ...
    assert.ok(content.includes('suggestions: []'), 'Return empty suggestions untuk query pendek')
  })
  ...
  await test('AC-08: engine — deepseek-reasoner sebagai primary', () => {
    ...
    assert.ok(content.includes('deepseek-reasoner'), 'Harus pakai deepseek-reasoner')
    assert.ok(content.includes('callDeepSeekReasoner'), 'Harus ada callDeepSeekReasoner')
  })

  await test('AC-08b: engine — Gemini 2.5 Flash-Lite sebagai fallback', () => {
    const content = readEngine()
    assert.ok(content.includes('gemini-2.5-flash-lite'), 'Fallback harus Gemini 2.5 Flash-Lite')
  ...
  await test('AC-08c: engine — model_version mencerminkan model yang dipakai', () => {
    ...
    assert.ok(content.includes('deepseek-reasoner'), 'deepseek-reasoner di default modelUsed')
  ...
  await test('AC-09: engine — guard jika tidak ada API key sama sekali', () => {
    const content = readEngine()
    assert.ok(
      content.includes('DEEPSEEK_API_KEY dan GEMINI_API_KEY keduanya tidak dikonfigurasi'),
      'Harus ada pesan error kedua key tidak ada'
    )
    assert.ok(content.includes('buildFallbackResult'), 'Harus panggil buildFallbackResult')
    assert.ok(content.includes('source: "error"'), 'Fallback source harus error')
  })
  ...
  await test('AC-10: vital signs — Hipertensi Krisis (sistolik ≥ 180)', () => {
    const content = readEngine()
    assert.ok(content.includes('Hipertensi Krisis'), 'Harus detect Hipertensi Krisis')
    assert.ok(content.includes('v.systolic >= 180'), 'Threshold sistolik ≥ 180')
  ...
  await test('AC-11: EMR page — state pemeriksaanSaran ada', () => {
    ...
    assert.ok(content.includes('pemeriksaanSaran'), 'Harus ada state pemeriksaanSaran')
  ...
  await test('AC-11b: EMR page — triggerAutocomplete debounce 400ms ke /api/cdss/autocomplete', () => {
    const content = readPage()
    assert.ok(content.includes('triggerAutocomplete'), 'Harus ada fungsi triggerAutocomplete')
    assert.ok(content.includes('400'), 'Debounce harus 400ms')
    assert.ok(content.includes('/api/cdss/autocomplete'), 'Harus hit endpoint autocomplete')
  })
  ...
  await test('AC-11c: EMR page — tombol RESET membersihkan semua autocomplete state', () => {
    ...
    assert.ok(resetBlock.includes('setSymptomSuggestions([])'), 'Reset harus clear suggestions')
    assert.ok(resetBlock.includes('setShowSuggestions(false)'), 'Reset harus hide dropdown')
    assert.ok(
  ...
  await test('AC-11d: EMR page — panel SARAN PEMERIKSAAN dengan 3 kategori (fisik, lab, penunjang)', () => {
    const content = readPage()
    assert.ok(content.includes('SARAN PEMERIKSAAN'), 'Harus ada label SARAN PEMERIKSAAN')
    ...
    assert.ok(content.includes('pemeriksaanSaran.penunjang'), 'Harus render penunjang')
  })

  await test('AC-11e: EMR page — dropdown SARAN GEJALA label ada', () => {
    const content = readPage()
    assert.ok(content.includes('SARAN GEJALA'), 'Dropdown harus ada label SARAN GEJALA')
  })
  ...
  if (fail > 0) {
    ...
    const skipNote =
      skip > 0 ? ` (${skip} skipped — butuh GEMINI_API_KEY untuk live embedding)` : ''
