...
const PATCHES = {
  ...
  30: (entry) => {
    ...
    pt.first_line = pt.first_line.filter(
      (d) => !d.drug.toLowerCase().includes("pseudoefedrin"),
    );
    const hasLoratadin = pt.first_line.some((d) =>
      d.drug.toLowerCase().includes("loratadin"),
    );
    if (!hasLoratadin) {
      pt.first_line.push({
        drug: "Loratadin",
    ...
    const hasCTM = pt.first_line.some(
      (d) =>
        d.drug.toLowerCase().includes("ctm") ||
        d.drug.toLowerCase().includes("klorfeniramin"),
    );
    if (!hasCTM) {
      pt.first_line.push({
  ...
  35: (entry) => {
    ...
    pt.first_line = pt.first_line.filter(
      (d) => !d.drug.toLowerCase().includes("ambroksol"),
    );
    ...
    const hasAmox = pt.second_line?.some((d) =>
      d.drug.toLowerCase().includes("amoksisilin"),
    );
    if (!hasAmox) {
  ...
  37: (entry) => {
    ...
    const hasBenzatin = pt.second_line?.some(
      (d) =>
        d.drug.toLowerCase().includes("benzatin") ||
        d.drug.toLowerCase().includes("penisilin"),
    );
    if (!hasBenzatin) {
      if (!pt.second_line) pt.second_line = [];
  ...
  39: (entry) => {
    ...
    const hasNebul = pt.first_line.some(
      (d) =>
        ...
        (d.drug.toLowerCase().includes("salbutamol") &&
          d.route?.toLowerCase().includes("nebul")),
    );
    if (!hasNebul) {
      // Insert nebulizer after MDI entry
      const mdiIdx = pt.first_line.findIndex(
        (d) =>
          d.drug.toLowerCase().includes("mdi") ||
          d.drug.toLowerCase().includes("inhaler"),
      );
      const nebEntry = {
...
const hasCovid = diseases.some(
  (d) => d.icd10 === "U07.1" || d.icd10?.startsWith("U07"),
);
...
console.log(
  `COVID-19 added  : ${covidAdded ? "Yes (U07.1)" : "No (already exists)"}`,
);
console.log("");
console.log("=== CHANGES ===");
changeLog.forEach((c) => {
  console.log(
    `  [ID ${c.id}] ${c.name} (${c.icd10}) — pharmacotherapy updated`,
  );
});
if (covidAdded) {
...
const backupPath = resolve(
  backupDir,
  `144_penyakit_puskesmas.pre-ppk2022.${ts}.bak.json`,
);
