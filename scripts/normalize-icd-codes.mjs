...
const toFix = diseases.filter(
  (e) => COMPOSITE_MAP[e.icd10] || SINGLE_FIX[e.icd10],
);
...
for (const entry of toFix) {
  ...
  if (hasSecondary) {
    console.log(
      `          icd10_secondary = ${JSON.stringify(mapping.secondary)}`,
    );
  }
...
console.log(
  `Extensions to add to icdx-extensions.json: ${NEW_EXTENSIONS.length}`,
);
NEW_EXTENSIONS.forEach((e) =>
  console.log(`  + ${e.code} → ${e.legacyCode} (${e.display})`),
);
...
copyFileSync(
  DISEASES_FILE,
  join(BACKUP_DIR, `144_penyakit.${timestamp}.bak.json`),
);
copyFileSync(
  EXT_FILE,
  join(BACKUP_DIR, `icdx-extensions.${timestamp}.bak.json`),
);
