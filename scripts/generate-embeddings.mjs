...
// Load penyakit DB
const db = JSON.parse(
  readFileSync(join(ROOT, "public", "data", "penyakit.json"), "utf-8"),
);
...
for (let i = 0; i < penyakit.length; i++) {
  ...
  try {
    ...
    process.stdout.write(
      `\r[${i + 1}/${penyakit.length}] ✓ ${d.icd10} ${d.nama.substring(0, 40)}`,
    );
  } catch (e) {
...
console.log(
  `Output: public/data/penyakit-vectors.json (${vectors.length} vectors)`,
);
