...
async function main(): Promise<void> {
  ...
  const debugRoutePath = new URL(
    "../src/app/api/auth/debug-env/route.ts",
    import.meta.url,
  );
  assert.equal(
    fs.existsSync(debugRoutePath),
    false,
    "Debug auth route file must be removed",
  );

  const authModule = await import("../src/lib/server/crew-access-auth");
  const passwordHash =
    await authModule.hashCrewAccessPassword("TestPassword#20260");
  ...
  try {
    ...
    assert.equal(
      ...
      200,
      "Hashed password login must return 200",
    );
    ...
    assert.equal(
      invalidDateRegisterResponse.status,
      400,
      "Impossible birth date must be rejected",
    );
    ...
    assert.equal(
      anonymousProfileResponse.status,
      401,
      "Anonymous profile request must return 401",
    );
    ...
    assert.equal(
      anonymousTtsResponse.status,
      401,
      "Anonymous TTS request must return 401",
    );
    ...
    const payload = (await authenticatedVoiceTokenResponse.json()) as Record<
      string,
      unknown
    >;
    ...
    assert.equal(
      "apiKey" in payload,
      false,
      "Voice token response must not include apiKey field",
    );
    assert.equal(
    ...
    const profileBeforePayload =
      (await authenticatedProfileBeforeUpdate.json()) as {
    ...
    ) as Array<{ username?: string; fullName?: string }>;
    assert.equal(
      savedProfiles[0]?.username,
      "security.test",
      "Profile file must persist username",
    );
    assert.equal(
    ...
    assert.equal(
      authenticatedTtsResponse.status,
      200,
      "Authenticated TTS request must return 200",
    );
    assert.equal(
