...
const updatedPenyakit = raw.penyakit.map((entry) => {
  ...
  if (update.kriteria_rujukan !== undefined)
    patched.kriteria_rujukan = update.kriteria_rujukan;
  patched.source =
    "PPK Dokter FKTP — KMK HK.01.07/MENKES/1186/2022 + 1936/2022";
  updatedCount++;
  return patched;
});
...
const covidExists = raw.penyakit.some(
  (e) =>
    e.icd10 === "U07.1" || (e.nama && e.nama.toLowerCase().includes("covid")),
);
...
console.log(
  `COVID-19 to add  : ${covidExists ? "No (already present)" : "Yes (DIS-COVID)"}`,
);
console.log("");
...
if (!APPLY) {
  ...
  for (const [id, patch] of Object.entries(PPK_2022_UPDATES)) {
    ...
    if (patch.terapi) {
      patch.terapi.forEach((t) =>
        console.log(`         + ${t.obat} ${t.dosis}`),
      );
    }
  }
  if (!covidExists) {
    ...
    console.log(
      `         pemeriksaan_fisik: ${COVID_ENTRY.pemeriksaan_fisik.length} items`,
    );
...
const finalPenyakit = covidExists
  ? updatedPenyakit
  : [...updatedPenyakit, COVID_ENTRY];
...
console.log(
  `       ${updatedCount} entries updated, ${covidExists ? 0 : 1} new entry added`,
);
