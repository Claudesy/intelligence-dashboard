...
function main(): void {
  ...
  const byName = new Map(
    anginaPlan.medications.map((item) => [item.name, item]),
  );
  ...
  assert.ok(
    anginaPlan.stockCoverageLabel.includes("di luar formularium disembunyikan"),
  );
  ...
  const anginaRiskByName = new Map(
    anginaWithRisks.medications.map((item) => [item.name, item]),
  );
  assert.ok(
  ...
  const appendicitisMeds = new Map(
    appendicitisPlan.medications.map((item) => [item.name, item]),
  );
  assert.ok(
  ...
  assert.ok(
    appendicitisPlan.supportive.some((item) =>
      item.includes("IV Line (NS/RL)"),
    ),
  ...
  const salpingitisMeds = new Map(
    salpingitisPlan.medications.map((item) => [item.name, item]),
  );
  assert.ok(
  ...
  const diabetesByName = new Map(
    diabetesPlan.medications.map((item) => [item.name, item]),
  );
  assert.equal(
    diabetesByName.get("Metformin HCl tablet 500 mg")?.prescriptionSlot,
    "utama",
  );
  assert.equal(
    diabetesByName.get("Glimepirid tablet 4 mg")?.prescriptionSlot,
    "utama",
  );
  assert.ok(
    diabetesByName
      ...
      ?.contraindications.some(
        (item) =>
          item.toLowerCase().includes("ginjal") ||
          item.toLowerCase().includes("asidosis"),
      ),
  ...
  assert.equal(
    normalizeDrugNameForPrescription("Amoxicillin"),
