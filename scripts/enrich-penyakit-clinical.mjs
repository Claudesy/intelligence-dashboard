...
if (!DRY_RUN && !API_KEY) {
  ...
  console.error(
    "Usage: GEMINI_API_KEY=your_key node scripts/enrich-penyakit-clinical.mjs",
  );
  process.exit(1);
}
...
const missing = entries.filter(
  (e) => !e.pemeriksaan_fisik?.length || !e.red_flags?.length,
);
...
console.log(
  `  - Missing red_flags: ${entries.filter((e) => !e.red_flags?.length).length}`,
);
console.log("");
...
if (DRY_RUN) {
  ...
  missing.forEach((e) => {
    ...
    console.log(
      `  [${e.id}] ${e.nama} (${e.icd10}) — missing: ${missingFields.join(", ")}`,
    );
  });
  console.log(
...
for (let i = 0; i < missing.length; i++) {
  ...
  process.stdout.write(
    `[${i + 1}/${missing.length}] ${entry.id} ${entry.nama} ... `,
  );
